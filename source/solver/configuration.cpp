...
void with_move(Tap_Result& result, Board_Position tap_position) {
	if (can_rotate(result.config.board[tap_position]))
		++result.config.action_count;
	result.top = result.config.board.get_gap();
	result.config.board.swap_with_gap(tap_position);
	if (result.config.board.get_gap() < result.top)
		result.top = result.config.board.get_gap();
	result.moved = true;
}

Tap_Result with_tap(const Configuration& input, Board_Position tap) {
	Tap_Result result { input, /* changed = */ true, /* moved = */ false, /* top = */ Board_Position() };
